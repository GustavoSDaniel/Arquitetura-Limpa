...
public class ValidandoCPF {
    //Validação padrao de cpf
    private String value;

    public ValidandoCPF(String taxNumber) throws Exception {
        setValue(taxNumber);
    }
    ...
    public void setValue(String value) throws Exception {
        if (!isValid(value)) throw new ValidandoCPFException(ErrorCodeEnum.ON0001.getMensagem(), ErrorCodeEnum.ON0001.getCodego());
        this.value = value;
    }
    ...
    private Boolean isValid(String taxNumber) throws Exception {
        ...
        if (taxNumber.replaceAll("[^0-9]", "").length() == 11 || taxNumber.replaceAll("[^0-9]", "").length() == 14){
            ...
            throw new ValidandoCPFException(ErrorCodeEnum.ON0001.getMensagem(), ErrorCodeEnum.ON0001.getCodego());
        }
    }
